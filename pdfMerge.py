# This script is largely based on https://www.thepythoncode.com/article/merge-pdf-files-in-python
# It is modified for automatic merging of a folders contents, instead of specifying input file names individually

#Import Libraries
from PyPDF4 import PdfFileMerger
import os,argparse

def merge_pdfs(input_folder: str, input_files: list, page_range: tuple, output_file: str, bookmark: bool = True):
    """
    ...
    for input_file in input_files:
        ...
        merger.append(fileobj=open("./" + input_folder + "/" + input_file, 'rb'), pages=page_range, import_bookmarks=False, bookmark=bookmark_name)
...
def parse_args():
    ...
    parser.add_argument('-i', '--input_files', dest='input_files', nargs='*',
                        type=str, required=True, help="Enter the path of the folder containing files to be combined")
    ...
    parser.add_argument('-b', '--bookmark', dest='bookmark', default=False, type=lambda x: (
...
if __name__ == "__main__":
    ...
    # Get file list
    path = "./" + args['input_files'][0] #Folder in same folder as script
    newlist = []
    ...
    merge_pdfs(
        input_folder=args['input_files'][0],input_files=newlist, page_range=page_range, 
